...
import {
  JanuaWebhookPayloadDto,
  JanuaWebhookEventType,
} from "./dto/janua-webhook.dto";
...
export class BillingController {
  ...
  async handleJanuaWebhook(
    ...
    try {
      ...
      this.logger.error(
        `Error processing Janua webhook: ${err.message}`,
        err.stack,
      );
  ...
  @ApiQuery({
    name: "country",
    required: false,
    description: "Country code for pricing (default: MX)",
  })
  ...
  async getSubscription(@TenantId() tenantId: string) {
    ...
    return {
      ...
      utilization: {
        learners:
          limits.learners === -1 ? 0 : (usage.learners / limits.learners) * 100,
        courses:
          limits.courses === -1 ? 0 : (usage.courses / limits.courses) * 100,
        dc3:
          limits.dc3_per_month === -1
            ? 0
            : (usage.dc3_this_month / limits.dc3_per_month) * 100,
  ...
  async getFeatures(@TenantId() tenantId: string) {
    ...
    const allFeatures = [
      ...
      {
        id: "dc3_generation",
        name: "Generación DC-3",
        description: "Constancias de habilidades laborales",
      },
      {
        id: "sirce_export",
        name: "Export SIRCE",
        description: "Exportación para STPS",
      },
      {
        id: "lft_plans",
        name: "Planes LFT",
        description: "Planes de capacitación Ley Federal del Trabajo",
      },
      {
        id: "open_badges",
        name: "Open Badges 3.0",
        description: "Credenciales verificables",
      },
      {
        id: "basic_analytics",
        name: "Analítica Básica",
        description: "Reportes de progreso",
      },
      {
        id: "advanced_analytics",
        name: "Analítica Avanzada",
        description: "Dashboards y métricas detalladas",
      },
      {
        id: "custom_branding",
        name: "Marca Personalizada",
        description: "Logo y colores propios",
      },
      {
        id: "api_access",
        name: "Acceso API",
      ...
      },
      {
        id: "ece_oc_toolkit",
        name: "ECE/OC Toolkit",
        description: "Herramientas para centros de evaluación",
      },
      {
        id: "sso_saml",
        name: "SSO SAML",
        description: "Single Sign-On empresarial",
      },
      {
        id: "scim_provisioning",
        name: "SCIM Provisioning",
        description: "Sincronización de usuarios",
      },
      {
    ...
    return allFeatures.map((feature) => ({
      ...feature,
      available: (limits.features as readonly string[]).includes(feature.id),
    }));
  }
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: "Get billing portal URL for subscription management",
  })
  async getBillingPortal(
    @TenantId() tenantId: string,
    @Body() body: { returnUrl: string },
  ) {
    const url = await this.billingService.getBillingPortalUrl(
      tenantId,
      body.returnUrl,
    );
    return { url };
